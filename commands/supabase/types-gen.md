...
```bash
pnpm exec supabase gen types typescript --project-id YOUR_PROJECT_ID > lib/database.types.ts
```
...
{
  "scripts": {
    "gen-types": "pnpm exec supabase gen types typescript --project-id $SUPABASE_PROJECT_ID > lib/database.types.ts",
    "gen-types:local": "pnpm exec supabase gen types typescript --local > lib/database.types.ts"
  }
}
```
