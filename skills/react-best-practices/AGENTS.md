...
const Analytics = dynamic(
  () => import('@analytics/react').then(m => m.Analytics),
  { ssr: false }
)
...
**With Fluid Compute:** LRU caching is especially effective because multiple concurrent requests can share the same function instance and cache. This means the cache persists across requests without needing external storage like Redis.
...
Reference: [https://swr.dev](https://swr.dev)
